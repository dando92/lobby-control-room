...
/* The presets sit at the lightness and the saturation of the default teal: an
   accent that stops being restrained stops being a selection bar and becomes a
   block of colour, which is the one thing the palette is built to avoid. */
...
/* White stops holding as soon as an accent is bright. The threshold sits well
   above the default teal, so an interface nobody customised is untouched. The
   rule is written twice — here and in the inline script of index.html, which
   cannot import it — and the two must stay in step. */
...
/* The default clears the properties instead of writing them, so tokens.css
   stays the one place the default accent is painted from. */
